...
try:
    ...
    cmd = [
        'git', '-C', r'C:\Users\kodep\Practice',
        'filter-branch', '-f',
        '--msg-filter', f'python "{script_path}"',
        '--', '--all'
    ]
    
    print(f"Running filter-branch on Practice repo...")
    ...
    if result.returncode == 0:
        ...
        # Verify
        cmd_verify = ['git', '-C', r'C:\Users\kodep\Practice', 'log', '--pretty=format:%B']
        ...
        print("3. git update-ref -d refs/original/refs/heads/master")
        print("4. git update-ref -d refs/original/refs/remotes/origin/master")
    else:
